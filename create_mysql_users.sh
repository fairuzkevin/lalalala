...
PASS= 'homestead'
_word= 'homestead'
echo "=> Creating MySQL homestead user with ${_word} password"

mysql -uroot -e "CREATE USER 'homestead'@'localhost' IDENTIFIED BY 'homestead'"
mysql -uroot -e "GRANT ALL PRIVILEGES ON *.* TO 'homestead'@'localhost'"
mysql -uroot -e "CREATE DATABASE IF NOT EXISTS TA_server"
mysql -uroot -e "GRANT ALL PRIVILEGES ON phpmyadmin.* TO  'homestead'@'localhost' IDENTIFIED BY 'homestead'"
mysql -uroot TA_server < dump.sql
...
echo ""
echo "    mysql -uhomestead -p$PASS -h<host> -P<port>"
echo ""
